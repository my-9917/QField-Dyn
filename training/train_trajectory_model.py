...
from .quantum_trajectory_loss import rollout_loss
...
validation_dataset = QuantumTrajectoryDataset(
    ...
    quantum_cache_path=config["data"]["validation_quantum_features"],
...
selected_rollout_loss = rollout_loss
model_class = QuantumTrajectoryModel
model_kwargs = {
...
source_checkpoint = torch.load(
    config["initialization"]["checkpoint"], map_location=device, weights_only=False
)
if config["initialization"].get("full_model", False):
    model.load_state_dict(source_checkpoint["model_state_dict"])
else:
    incompatible = model.load_state_dict(
        source_checkpoint["model_state_dict"], strict=False
    )
    expected_missing = {
        "quantum_projection.0.weight",
        "quantum_projection.0.bias",
        "quantum_projection.2.weight",
        "quantum_projection.2.bias",
    }
    if set(incompatible.missing_keys) != expected_missing or incompatible.unexpected_keys:
        raise ValueError(f"unexpected initialization mismatch: {incompatible}")
...
def calculate_losses(batch):
    ...
    _, coordinate, bond, angle = selected_rollout_loss(
        model,
        batch,
        ...
        with torch.no_grad():
            for batch_index, batch in enumerate(validation_loader, start=1):
                ...
                validation_total += counts[0] * np.concatenate(
                    ([float(np.sum(loss_weights * component_values))], component_values)
                )
            ...
            torch.save(
                {
                    ...
                    "source_checkpoint": config["initialization"]["checkpoint"],
