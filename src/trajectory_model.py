...
class QuantumTrajectoryModel(nn.Module):
    def __init__(self, hidden_dim=128, layer_count=3, quantum_feature_dim=132):
        ...
        nn.init.zeros_(self.quantum_projection[-1].weight)
        nn.init.zeros_(self.quantum_projection[-1].bias)
    ...
    def forward(
        ...
        ligand_hidden = (
            ...
            + self.speed_embedding(
                torch.linalg.vector_norm(ligand_velocity, dim=-1, keepdim=True)
            )
        ...
        bond_vector = ligand_coordinates[bond_source] - ligand_coordinates[bond_target]
        bond_distance = torch.linalg.vector_norm(bond_vector, dim=-1, keepdim=True)
        ...
        cross_vector = protein_coordinates[cross_protein] - ligand_coordinates[cross_ligand]
        cross_distance = torch.linalg.vector_norm(cross_vector, dim=-1, keepdim=True)
