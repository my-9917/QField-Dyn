...
- T4：1000 ps 间隔和 490 步超出已有未来监督范围，使用同一框架中的观测历史随机动力学专家，重采样成对平移/旋转创新，并加入质心均值回归、刚体碰撞约束和 XTC 写回后的极端重叠修正。T1/T2 在候选选择之后附加观测动力学增强：从观测段重采样成对平移/旋转增量，叠加质心均值回归与口袋偏移半径约束，经碰撞安全投影并以二分法把平均步速校准到观测段水平；内部构象仅在不加剧蛋白碰撞时从模型候选移植。该步骤用于修正仅靠条件均值 rollout 时输出接近静态的问题；其随机性由 `config/model.json` 的 `seed`（20260816）加任务盐值驱动，重复推理逐元素一致。
...
单体系接口测试（T1/T2 参考输出由当前代码生成并含观测动力学增强；T3 参考保持冻结版数值，最大差异不超过一个 XTC 量化间隔以内）：
...
远程 NVIDIA A800 全量实测 217.0 秒 生成并核对 95 个体系，折合平均约 2.28 秒/体系；该平均值包含批处理与全量核对开销，不是独立单体系基准。T1/T2 的观测动力学增强（含二分速度校准）使这两档单体系耗时高于 T3/T4。进程峰值内存约 1.54 GB，PyTorch 峰值分配/保留显存约 39.0/52.4 MB；建议至少提供 1 个 CPU 核、4 GB 可用内存和 2 GB 可用显存。该建议不是多型号硬件上测得的最低边界。
...
- T1/T2 于冻结提交后修复了输出活动性不足（近静态）问题：本复现包的 T1/T2 输出含观测动力学增强，随包提交的 `GOAI_pred_xxxxxm429` 亦由当前代码在同一命令下生成，两包同源一致；T3/T4 与冻结版逐坐标一致。`examples/reference_t1/t2.npz` 已按当前代码重新生成。
